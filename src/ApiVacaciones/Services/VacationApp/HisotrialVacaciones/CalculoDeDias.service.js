 ...
 */
const calcularDiasSuspendidosEnAnio = (suspensiones, anioActual) => {
  if (!suspensiones || suspensiones.length === 0) return 0;
  let diasSuspendido = 0;

  suspensiones.forEach((s) => {
    const fInicio = dayjs(s.fechaInicioSuspension);
    const fFin = dayjs(s.fechaFinSuspension);
    const inicioAnio = dayjs(`${anioActual}-01-01`);
    const finAnio = dayjs(`${anioActual}-12-31`);

    if (fInicio.year() === anioActual || fFin.year() === anioActual || (fInicio.year() < anioActual && fFin.year() > anioActual)) {
      const startOverlap = fInicio.isAfter(inicioAnio) ? fInicio : inicioAnio;
      const endOverlap = fFin.isBefore(finAnio) ? fFin : finAnio;
      const diff = endOverlap.diff(startOverlap, "day") + 1;
      if (diff > 0) diasSuspendido += diff;
    }
  });

  return Math.round((diasSuspendido * 20) / 365);
};
...
const getDiasAcumuladosAnioAtrasadoCompleto = (diasPorAnio) => {
  ...
  contadorDiasAcuPorAnio = Math.min(20, Math.round((diasPorAnio * 20) / 365));
...
const getDiasAnioEnCursoIncompleto = (anio) => {
  ...
  const diasAcumulados = Math.min(20, Math.round(sumatoria));
...
const generarPayloadPorPeriodoAniosAtrasados = (data) => {
  ...
  periodoVacaciones.forEach((anio, index) => {
    ...
    const descuentoSuspension = calcularDiasSuspendidosEnAnio(data.suspensiones, anio);
    diasPeriodo = Math.max(0, diasPeriodo - descuentoSuspension);
    sumatoriaCalculo = Math.max(0, sumatoriaCalculo - descuentoSuspension);
...
const generarPayloadPeriodoAnioEncurso = (data) => {
  ...
  diasAcumuladosAnioEncurso.sumatoria = Math.max(0, diasAcumuladosAnioEncurso.sumatoria - descuentoSuspension);
...
export const generarPayloadPeriodoAnioEnCursoCambioDeAnio = (data) => {
  ...
  periodoVacaciones.forEach((periodo, index) => {
    ...
    const descuentoSuspension = calcularDiasSuspendidosEnAnio(data.suspensiones, periodo);
    diasPeriodo = Math.max(0, diasPeriodo - descuentoSuspension);
    sumatoriaCalculo = Math.max(0, sumatoriaCalculo - descuentoSuspension);
...
const generarPayloadAnioEnCursoNoInicio = (data) => {
  ...
  // Calcular los días acumulados en el año en curso
  let diasAcumuladosAnioEncurso = getDiasInicioAnioEnCurso(data.fechaIngreso);
  const descuentoSuspension = calcularDiasSuspendidosEnAnio(data.suspensiones, anioEnCurso);
