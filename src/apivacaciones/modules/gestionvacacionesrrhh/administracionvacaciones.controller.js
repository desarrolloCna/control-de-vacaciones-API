...
export const cancelarSolicitudAutorizadaController = async (req, res) => {
    try {
        const { idSolicitud, fechaResolucion, motivoReprogramacion, diasADevolver, idUsuarioSession, usuarioSession } = req.body;
        const result = await cancelarSolicitudAutorizadaService(idSolicitud, fechaResolucion, motivoReprogramacion, diasADevolver, idUsuarioSession, usuarioSession);
        const responseData = {
            status: 200,
